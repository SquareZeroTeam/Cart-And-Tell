...
export class MessagesGateway implements OnGatewayConnection {
  ...
  async endLivestream(
    ...
    try {
      let userObjPartial = this.jwt.decode(client.request.headers.authorization.split(' ')[1]);
      userObj = await this.prisma.user.findUnique({ where: { id: userObjPartial.id }, include: { merchant: true } }) as userObj;
    } catch (error) {
      throw new ForbiddenException('You are not authorized to access this resource');
    }
