...
const {
  ...
  pending,
  refresh,
} = await useLazyFetch<[CartObj]>(`${API}/user/${userObj.id}/cart`, {
  headers: {
...
async function increment(e: Event) {
  ...
  const data = await $fetch<any>(
    `${API}/user/${userObj.id}/cart/${id}/increment`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
async function decrement(e: Event) {
  ...
  const id = buttonDiv.id;
  const data = await $fetch<any>(
    `${API}/user/${userObj.id}/cart/${id}/decrement`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
async function remove() {
  ...
  if (data) {
    cart.value = cart.value?.filter(
      (product) => !checkedItems.value.includes(product.id as never)
    ) as [CartObj];
  }
  checkedItems.value = [];
}
...
<template>
  <div class="bg-gray-100">
    ...
    <div class="container mx-auto flex justify-center mt-2">
      ...
      <div
        class="container mx-auto justify-center items-center w-full mt-[4em] lg:pr-48 lg:pl-48 h-screen"
      >
          ...
          <div class="shadow-sm mt-8 rounded-md bg-white">
            <div class="flex justify-center">
              <img
                ...
                alt=""
                class="h-36 rounded-md m-4 sm:hidden"
              />
            </div>
            <div class="flex items-center bg-white productItem">
              <input
                ...
                :value="item.id"
                class="flex-shrink-0 ml-12 h-4 w-4 rounded-md text-[#2563EB] focus:ring-[#2563EB] ring-1 ring-offset-1 ring-offset-white"
              />
              <img
                :src="item.product.image"
                alt=""
                class="h-36 rounded-md m-4 hidden sm:block"
              />
              ...
              <div class="flex justify-end text-lg ml-auto">
                <div class="flex justify-center items-center">
                  <button
                    :id="item.id.toString()"
                    @click="decrement"
                    class="px-4 py-2 bg-gray-300 text-black"
                  >
                    -
                  </button>
                ...
                <p class="mt-4 mb-4 mr-10 ml-10 text-[#2563EB] font-semibold">
                  ₱{{ item.product.amount }}
                </p>
              </div>
            </div>
        ...
        <div class="flex justify-between">
          <button
            @click="checkout"
            class="bg-blue-600 text-white font-bold px-6 py-2 border-2 border-transparent rounded-full my-4"
          >
            Checkout
          </button>
          <button
            ...
            @click="remove"
            class="bg-red-600 text-white font-bold px-6 py-2 border-2 border-transparent rounded-full my-4"
          >
            Delete
          </button>
        </div>
