<script setup lang="ts">
    ...
    const {data:merchants} = await useFetch<any>((query) ? `${API}/merchant/verified?category=${query}`:`${API}/merchant/verified`);
</script>
<template>
    <div clas="container">
        <Header/>
                    ...
                    <div v-for="merchant in merchants" class="w-full gap-4 bg-[#282F7A]">
                        <div class="h-24 flex w-full ">
                            ...
                            <div class="flex justify-between items-center w-full">
                                <NuxtLink :to="`/merchants/${merchant.id}`">
                                    <h2 class="text-2xl font-bold underline m-[1rem] mt-[2.5rem] text-white hover:text-[#6DB7FB]">{{ merchant.name }}</h2>
                                </NuxtLink>
                            </div>
                        </div>
                        <div class="rounded-md block sm:flex justify-between mb-4 py-4">
                            <p class="w-[50rem] px-4 text-justify text-white mt-12 font-medium">{{ merchant.description }}</p>
