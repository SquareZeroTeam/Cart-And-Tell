<script setup lang="ts">
    ...
    const formData = reactive<{name:string,image:File|null,description:string,amount:number}>({
        name: "",
        image: null,
        description:"",
        amount:NaN,
...
const prevImage = ref(product.value!.image);
    async function update() {
      ...
      loading.value = true;
            ...
            const data = await $fetch<{message:string,id:number}>(`${API}/merchant/${useUserObj().value.merchant!.id}/products/${productId}`,{
                method: 'PATCH',
                headers: {
            ...
            if (!isError) {
              loading.value = false;
                await navigateTo(`/profile/${id}/products/${data!.id}`);
            }
        }
</script>
      ...
      <div class="lg:flex lg:items-center lg:justify-center lg:ml-8">
        ...
        <div class="flex justify-center items-center">
            <div class="flex items-center justify-center w-full relative">
                <img @click="(open as any)" :src="prevImage" alt="" class="absolute w-48 cursor-pointer">
        ...
        <div class="lg:ml-16 mt-4">
          ...
          <textarea v-model="formData.name" class="border-2 text-4xl font-bold w-full" :placeholder="`Product Name: ${product?.name}`"></textarea>
          <input v-model="formData.amount" type="number" class="text-xl font-semibold text-gray-700 mb-6 border-2" :placeholder="`Product Amount: ${product?.amount}`">
          <!-- <p class="text-3xl text-[#282F7A] font-bold mb-12">₱{{ product.amount }}</p> -->
          <textarea v-model="formData.description" class="border-2 w-full text-lg text-gray-600 leading-relaxed mb-6" :placeholder="`Product Description: ${product?.description}`"></textarea>
          <!-- <p class="text-lg text-gray-600 leading-relaxed mb-6">{{ product.description }}</p> -->

          <div class="flex items-center space-x-4">
            <button v-if="!loading" @click="update" class="flex justify-center items-center bg-[#282F7A] border-2 border-[#282F7A] w-36 h-12 text-white">
              <p class="text-md font-bold rounded-full">Edit Product</p>
            </button>
            
            <button v-else class="flex justify-center items-center bg-[#282F7A] border-2 border-[#282F7A] w-36 h-12 text-white">
              <svg
            aria-hidden="true"
            role="status"
