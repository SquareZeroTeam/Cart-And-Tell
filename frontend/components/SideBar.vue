...
<template>
  <div class="bg-white">
    <div class="flex flex-col items-center mt-8 gap-4 mr-4 ml-12">
      <NuxtLink to="/">
        <button
          class="w-44 h-[3rem] bg-[#6DB7FB] rounded-sm text-white font-bold text-xl mt-0.5"
        >
          <p>Home</p>
        </button>
      </NuxtLink>
      ...
      <div v-if="userObj.loggedIn && userObj.isMerchant && !userObj.merchant">
        <NuxtLink to="/registration">
          <button
            class="w-44 h-[4rem] bg-[#6DB7FB] rounded-sm text-white font-bold text-xl mt-0.5"
          >
